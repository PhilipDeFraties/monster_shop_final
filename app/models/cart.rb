class Cart
  ...
  def subtotal_of(item_id)
    @contents[item_id.to_s] * adjust_price(item_id)
  end
  ...
  def apply_discount(item_id, discounted_percentage)
    ((100 - discounted_percentage) * 0.01) * Item.find(item_id).price
  end

  def adjust_price(item_id)
